...
dt, T, Nsamp = 0.0005, 5, 200
t_param = [dt, T, Nsamp]
sys_param = [m, g, l, b]

# System responses:
xt, t_eval = utils_data.SimplePendulum(x0, t_param, sys_param)
...
lam = 15      # lam is our sparsification knob.
...
beta = utils.sparsifyDynamics(library=D_stoch, target=(dt*L_drift), lam=0.012) 
...
H_a = 0.5*xt_mean[1,:]**2 - (g/l)*np.cos(xt_mean[0,:]) 
H_i = Hfun(xt_mean[0], xt_mean[1]) 

""" Lagrangian time series """
L_a = 0.5*xt_mean[1,:]**2 + (g/l)*np.cos(xt_mean[0,:]) 
...
np.savez('results/results_stochastic_pendulum', Xi=Xi, 
