...
# -*- coding: utf-8 -*-
"""
-- This code is for "discovering Lagrangian of Stochastic Duffing Oscillator" 
...
x0 = np.array([0.4, 0])
m, k, alpha, b = 1, 1000, 5000, 1
dt, T, Nsamp = 0.0001, 1, 200
t_param = [dt, T, Nsamp]
sys_param = [m, k, alpha, b]

# System responses:
xt, t_eval = utils_data.Duffing(x0, t_param, sys_param)
...
plt.suptitle('Stochastic Duffing Oscillator', y=0.95)
...
x, y = sym.symbols('x, y')
xvar = [x, y]
D, nd = utils.library_sdof(xvar,polyn=4,harmonic=0)
...
beta = utils.sparsifyDynamics(library=D_stoch, target=(dt*L_drift), lam=0.2)
...
H_a = 0.5*xt_mean[1,:]**2 + 0.5*(k/m)*xt_mean[0,:]**2 + 0.25*(alpha/m)*xt_mean[0,:]**4 
H_i = Hfun(xt_mean[0], xt_mean[1]) 

""" Lagrangian time series """
L_a = 0.5*xt_mean[1,:]**2 - 0.5*(k/m)*xt_mean[0,:]**2 - 0.25*(alpha/m)*xt_mean[0,:]**4 
...
np.savez('results/review_results_stochastic_Duffing'+str(Nsamp), Xi=Xi, 
